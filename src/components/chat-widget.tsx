...
type Channel = 'WhatsApp' | 'Messenger' | 'Instagram' | 'LinkedIn' | 'Email' | 'Interne';
...
const initialConversations: Conversation[] = [
  ...
  { id: 'conv4', name: "Équipe Support", userIds: ['user0', 'user4'], messages: [{ id: 'msg4', text: 'Réunion prévue à 15h', senderId: 'user4', timestamp: '09:30' }], contactType: 'Interne', channel: 'Interne' },
...
const platformIcons: Record<Channel, { icon: React.FC<any>, color: string }> = {
    Interne: { icon: MessageCircle, color: "bg-primary" },
            ...
            {!activeConversation || !activeContact ? (
                <div className="h-full flex flex-col items-center justify-center text-center p-8">
                    ...
                    <div className="mt-6 text-left text-sm text-muted-foreground space-y-2">
                        ...
                        <p className="flex items-center gap-2"><Check className="h-4 w-4 text-green-500"/> Discussions Internes</p>
                    </div>
                </div>
            ) : (
                <>
...
export function ChatWidget() {
    ...
    return (
        <>
            <Button 
                className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-lg z-40 text-white animate-bounce-gentle hover:animate-none"
